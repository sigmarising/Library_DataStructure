...
public:
    ...
    bool BookList_Borrow(const string & id_pre, const string & id_person, bool Subscribe);	// 借书 读入两个id信息 并且自动判断 进行借书动作还是预约动作(当 Subscribe 为 TRUE 时才进行预约 否则不进行预约) 借书时候自动分配完整id书
